...
static void mallinfo_check(struct mallinfo *start)
{
...
static void test_wakeup_thread(int num)
{
	int i;
	int ret;
...
void test_wakeup(int all)
{
	...
	/* create all threads */
	for (ii = 0; ii < NTHREADS; ii++) {
	  child[ii] = thread_create((void (*)(void *))test_wakeup_thread,(void *)ii);
		assert(thread_ret_ok(child[ii]));
	}
out:
...
static void test_wait_thread(int num)
{
...
static void test_wait_kill_thread(Tid parent)
{
...
static void test_wait_parent_thread(Tid parent)
{
...
static void test_cv_signal_thread(unsigned long num)
{
	int i;
	struct timeval start, end, diff;
...
static void test_cv_broadcast_thread(unsigned long num)
{
	int i;
	struct timeval start, end, diff;
